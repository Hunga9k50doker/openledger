...
async function checkAndClaimTier(token, proxy, index) {
  const agent = newAgent(proxy);

  try {
    const response = await axios.get("https://rewardstn.openledger.xyz/api/v1/tier_details", {
      headers: {
        ...headers,
        Authorization: "Bearer " + token,
      },
    ...
    let { tierDetails, tireId } = response.data.data;
    if (tierDetails?.length <= 0) return;
    tierDetails = tierDetails.filter((t) => t.tireId <= tireId && !t.claimStatus);
    for (const tierDetail of tierDetails) {
      logger.info(`▸Account ${index} Claiming tier: ${tierDetail.tireName}`);
      const responseClaim = await axios.put(
        `https://rewardstn.openledger.xyz/api/v1/claim_tier`,
        { tireId: tierDetail.tireId },
        {
          headers: {
            ...headers,
            Authorization: "Bearer " + token,
      ...
      if (responseClaim.data.status == "SUCCESS") {
        logger.info(`▸Account ${index} Claimed tier: ${tierDetail.tireName} | Reward: ${tierDetail.value}`);
      }
    }
    return response.data.data;
  } catch (error) {
    log.error("Error claiming tier reward:", error.message || error);
    return null;
  }
}
  ...
  const accountsProcessing = wallets.map(async (address, index) => {
    ...
    while (!isConnected) {
      try {
        ...
        await checkAndClaimTier(token, proxy, index + 1);
